...
1. `structural_validation.passed=true`。
...
证据目录的每个 `rows[]` 明确给出 `row_index`、原始 `cells` 和可直接使用的 `value_pointers`。定量引用应与相应 `value_pointers` 完全一致；`source_location_not_row_index` 不是 row_index。请逐字核对原值、单位及期间。引用错误须依据 `value_pointers` 指出正确候选，不能要求保留已经判定无效的行号或把项目名称当作字段。

1. 任一通过条件不满足，`verdict` 必须为 `revise`。
2. 每个问题只描述一个具体缺陷，指出目标位置和所需动作。
3. `route` 只能选择：`finish`、`replan`、`rewrite`。
4. 需求、口径、计算或证据需要改变时选择 `replan`；只有表达问题选择 `rewrite`；全部通过才选择 `finish`。
